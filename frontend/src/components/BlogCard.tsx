  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 lg:gap-8 m-8">
      {Blogs.map((blog, index) => {
        const isWishlisted = wishlist.blogs.includes(blog.id);
    ...
    `}
              ...
              <div className="absolute top-4 left-4">
                <span className="bg-[var(--primary-color)] text-white text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-full shadow-md">
                  {blog.category}
                </span>
              </div>
            </div>
            <div className="flex-1 flex flex-col justify-between p-6 gap-4 ">
              <div className="flex flex-wrap items-center text-xs font-medium text-gray-500 justify-between ">
              ...
              <div className="flex flex-col gap-2">
                <h3 className="text-xl font-bold text-gray-900 line-clamp-2 leading-tight group-hover:text-[var(--primary-color)] transition-colors">
                  {blog.title}
                </h3>
