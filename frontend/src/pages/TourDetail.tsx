...
function TourDetail() {
  ...
  const [selectedPackage, setSelectedPackage] = useState(Tour?.packages[0]);
      ...
      <section className="relative group rounded-lg overflow-hidden">
        ...
        <div className="flex justify-center  p-5 gap-10 absolute bottom-0 w-full">
          ...
          >
            ...
            <p className="text-white">Maximum Altitude: {Tour?.maxAltitude}</p>
          </Button>
          <Button
            variant="transparentButton"
            className="flex items-center gap-2 w-[230px]"
          >
            ...
            <p className="text-white">Group Size: {Tour?.groupSize.min}-{Tour?.groupSize.max} pax</p>
          </Button>
        </div>
      </section>
  ...
  onClick={() =>
    setValue(
      "numberOfTraveller",
      Math.min(numberOfTraveller + 1, Tour?.groupSize.max ?? 10)
    )
  }
/>
                </div>
